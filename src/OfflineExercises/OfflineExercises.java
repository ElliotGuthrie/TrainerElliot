...
public class OfflineExercises {
	...
	{
		String output, sectionStart, sectionEnd;
		output = sectionStart = sectionEnd = "";
		
		for(int i = 0; i < a; i++)
		{
			sectionStart = input.substring(i, i + 1);
			output += sectionStart;
		}
		for(int j = input.length() - a; j < input.length(); j++)
		{
			sectionEnd = input.substring(j, j + 1);
			output += sectionEnd;
		}
		return output;
	}
	...
	{
		String output, inputPlus;
		output = "";
		inputPlus = input + " ";
		
		for(int i = 0; i < inputPlus.length() - 1; i++)
		{
			if(inputPlus.charAt(i) == inputPlus.charAt(i + 1))
				continue;
			else
				output += inputPlus.charAt(i);
		}
		return output;
	}
	...
	public int fibonacci(int input) 
	{
		...
		int firstNum, secondNum, firstFibo, secondFibo, fiboNum;
		
		firstFibo = fiboNum =  0;
		secondFibo = 1;
		
		fiboArray.add(firstFibo, 0);
		fiboArray.add(secondFibo, 1);
		
		if(input <= 1)
			fiboNum = fiboArray.get(input);
		else if(input > 1)
		{
			for(int i = 1; i < input; i++)
			{
				firstNum = fiboArray.get(i);
				secondNum = fiboArray.get(i - 1);
				fiboNum = firstNum + secondNum;
				fiboArray.add(fiboNum);
